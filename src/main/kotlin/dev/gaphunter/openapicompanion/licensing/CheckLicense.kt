 ...
 * already used in ansible-companion (a real assigned code) and
 * api-security-companion (also a real assigned code, Freemium). The
 * certificate/crypto logic below is JetBrains's own boilerplate for
 * verifying a license was genuinely signed by them -- do not modify it
 * when adapting this file, only PRODUCT_CODE is project-specific.
 *
 * This plugin is 100% Paid, no free tier -- unlike the Freemium plugins
 * above (where only a specific Pro feature gates on isLicensed()),
 * every real entry point in this plugin calls into this check: the
 * reference resolvers (go-to-definition never resolves without a
...
object CheckLicense {
    ...
    private fun showRegisterDialog(productCode: String, message: String?) {
        ...
        if (registerAction != null) {
            // Same API confirmed across every other plugin in this catalog that
            // ports this file: invokeAction is ActionUtil's own documented
            // replacement for the deprecated Component-based overload, accepts
            // DataContext, and isn't flagged Experimental/OverrideOnly/Internal.
